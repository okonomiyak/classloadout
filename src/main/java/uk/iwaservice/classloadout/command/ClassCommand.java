...
/**
 * {@code /class} command tree. All loadout-class operations enter the
 * server exclusively through here (no C2S packets), so the permission
 * checks below are the single line of defense.
 */
public final class ClassCommand {
    ...
    /** Opens the OP-only class editor client-side; permission already enforced by the command node. */
    ...
    /** Creates or (if the id already exists) overwrites a class definition in one atomic command. */
    ...
    private static int select(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        manager.select(ctx.getSource().getServer(), player, id);
        ctx.getSource().sendSuccess(() -> Component.translatable("classloadout.msg.class_selected", def.name()), false);
        return 1;
    }

    private static int clear(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ServerPlayer player = ctx.getSource().getPlayerOrException();
        LoadoutManager.get(ctx.getSource().getServer()).clearSelection(ctx.getSource().getServer(), player);
