...
| `/class whitelist ammo <slot> <item> <ammoItem> <count>` | Attach an ammo grant to an already-whitelisted item: equipping it (on respawn, or immediately via the loadout station/locker) also gives `count` of `ammoItem` into the player's general inventory. `count 0` clears the grant (sent by the whitelist editor's ammo popup, not typed by hand) | OP (level 2+) |
...
- **One exception**: opening the preset/whitelist/protected-items/spawn-kit/hammer-blocks editor screens. `/class editor`, `/class whitelist`, `/class protect`, `/class spawnkit` and `/class hammerblocks` run on the server (permission-checked there), which then sends a tiny trigger packet back to that one client to open the screen - so the client never has to re-derive its own permission level.
