...
4. 強制装備したスロットの自動ロック機能(本人は`assign`/`select`/`clear`で変更不可、`forceassign ... minecraft:air`で解除、ロードアウト画面に赤枠表示)
5. **重要なバグ修正**: `ServerEvents#equipLoadout`のホワイトリストチェックが、ロックされた(=ホワイトリストを意図的にバイパスする)スロットにも適用されてしまい、強制装備した非ホワイトリストアイテムが次のリスポーンで消える不具合を修正

以後の変更は再びこのセクションで追跡すること。

**旧メモ(v0.5.2リリース時点)**:v0.5.1に続けて、ユーザー指示「ステーション/ロッカーの即時装備でもクリアするようにして」に対応: `ServerEvents.equipLoadout(ServerPlayer)`(即時装備側)でも`death.clearInventoryOnDeath`が有効なら`clearInventoryExceptProtected`をリスポーン時と同じ順序(クリア→装備→弾薬付与)で実行するようにした。これでステーション/ロッカーの即時装備はリスポーンと完全に同じ挙動(インベントリクリア+弾薬付与)になった。README.md/README.ja.mdの「Death Behavior」節も更新済み。詳細・実地確認項目は下記「2026-08-15セッションのまとめ」を参照(v0.5.1の項目に追記する形で記載)。
...
v0.5.2に続く大型セッション。ユーザーの短い日本語指示を都度確認質問(AskUserQuestion)で仕様確定させながら、ロードアウトシステムをかなり拡張した。時系列順。

### 1. 弾薬付与の複数エントリ対応

ユーザー「玉設定で2個以上の種類のアイテムを渡せれるようにしたい」。従来`AmmoGrant(ammoItem, count)`レコードで1アイテムにつき弾薬付与は最大1種類だった。

- `LoadoutManager`の`ammoGrants`を`Map<LoadoutSlot, Map<ResourceLocation/*item*/, AmmoGrant>>`から`Map<LoadoutSlot, Map<ResourceLocation/*item*/, Map<ResourceLocation/*ammoItem*/, Integer/*count*/>>>`に変更、`AmmoGrant`レコード自体を削除。
- `/class whitelist ammo <slot> <item> <ammoItem> <count>`のシグネチャは維持しつつ意味を「1エントリの追加/削除」に変更(`count 0`でそのammoItemだけ削除、他のエントリには影響しない)。
- `AmmoGrantScreen`を全面刷新: 既存の弾薬付与一覧を小さなアイコングリッドで表示(クリックで個別削除)、下のピッカー+個数欄から新規追加。
- `LoadoutSyncPacket.AmmoGrantEntry`は元々フラットな(slot, item, ammoItem, count)リストだったため、ワイヤーフォーマット自体の変更は不要だった。

### 2. 強制装備コマンド一式(`forceselect`/`forceassign`とその全員・チーム版)+専用GUI

ユーザー「ロードアウトを強制させれるように」→「頭だけ強制するとかできない？」→「装備も強制できる？」→「空だと全プレイヤーで」→「teamによって装備を矯正できるように」と段階的に要件が広がった。

- `/class forceselect <player> <id>`・`/class forceassign <player> <slot> <item>`(OP限定、1人のプレイヤーを指定)。プリセット/防具含む全スロットが対象。ホワイトリストは無視(OP操作のため)。
- `/class forceselectall <id>`・`/class forceassignall <slot> <item>`: 対象を指定しない代わりオンライン中の**全員**に適用。
- `/class forceselectteam <team> <id>`・`/class forceassignteam <team> <slot> <item>`: バニラのスコアボードチーム(`TeamArgument`)のオンライン中の**全メンバー**に適用。
- 上記6コマンドの重複ロジックは`applyForceSelect`/`applyForceAssign`ヘルパーに集約(1人版・全員版・チーム版が共通で呼ぶ)。
- 新規GUI`ForceLoadoutScreen`(`/class force`で開く、`ClassEditorScreen`のナビバーにも追加): 対象プレイヤー名欄+チーム名欄(チーム名が優先、両方空欄なら全員)、プリセット[適用]ボタン、10個のスロットアイコン(クリックでそのスロット用ホワイトリスト制限付きピッカーを開く)。
  - チーム名欄の下にはクライアント側スコアボード(`minecraft.level.getScoreboard().getPlayerTeams()`、既にバニラ同期済み)から取得したチーム名のクリック選択ボタンを表示。
  - **試行錯誤の記録**: 最初にオンラインプレイヤーのクリック選択ボタンも実装したが、ユーザーに「このボタンいらない」と却下されて削除。一方チーム名のクリック選択ボタンは「GUIで」の依頼で明示的に追加(混同しないよう注意——プレイヤー版とチーム版で判断が分かれた)。
  - **見つかったUIバグ**: 対象プレイヤー名が空欄の状態でスロットアイコンをクリックしても何も起きない(サイレントに無視)仕様になっており、ユーザーから「ボタンをおしてもロードアウト選択できる画面にいかない」と報告された。ピッカーは常に開き、コマンド送信だけ空欄チェックする方式に修正済み。

### 3. ロードアウトスロット拡張(6→10): 第二ガジェット+防具4部位

ユーザー「第二ガジェットをください」→「装備も変えられるようにして」で2段階拡張。

- `LoadoutSlot`列挙型に`GADGET2`・`HELMET`・`CHESTPLATE`・`LEGGINGS`・`BOOTS`を追加。各値が`hotbarIndex()`(ホットバー装備用、0〜5)か`equipmentSlot()`(`net.minecraft.world.entity.EquipmentSlot`、防具用)のどちらか一方を持つ設計にし、`ServerEvents#equipLoadout`が両方を自動判別して装備するようにした(`player.getInventory().setItem`と`player.setItemSlot`を使い分け)。
- `PersonalLoadout`・`ClassDefinition`・`LoadoutSyncPacket`(`Entry`/`PersonalData`/`Whitelists`)を全て10スロット対応に拡張。NBTタグ名を追加するだけなので既存ワールドとの後方互換は保たれる(新スロットは未設定として読み込まれる)。
- `ClassDefinition`に`withName`/`withIcon`/`withSlot`ヘルパーメソッドを追加(`PersonalLoadout`の`withSlot`と同型)。12引数コンストラクタを`ClassCommand.saveSlot`の10ケースswitch文で毎回組み立てるのは転記ミスのリスクが高すぎたため、リファクタリングして簡略化。
- UI側(`ClassEditorScreen`・`LoadoutScreen`)はギア6スロットの下に防具4スロットの2段目の行を追加するレイアウト変更。`WhitelistEditorScreen`のスロットタブは元々`LoadoutSlot.values()`ベースの汎用実装だったため自動的に10タブになった(コード変更不要)。

### 4. 強制装備スロットの自動ロック+GUIでの赤枠表示

ユーザー「強制して変更不可にできない？」→(確認質問で「強制されたスロットだけロック」を選択)→「強制装備はGUIで周りを赤くして」。

- `LoadoutManager`に`Map<UUID, Set<LoadoutSlot>> lockedSlots`を追加(NBT永続化)。`isLocked`/`lockSlot`/`unlockSlot`/`getLockedSlots`。
- `applyForceAssign`/`applyForceSelect`(強制装備の共通ヘルパー、上記2番参照)が、設定した値が非nullなら該当スロットを自動ロック。`forceassign ... minecraft:air`で既にロック中のスロットに送ると自動的にアンロックされる(専用の解除コマンドは作らず、既存の「air=クリア」慣習を流用)。
- 自己サービス側(`ClassCommand.assign`/`select`/`clear`)がロックを尊重するよう変更:
  - `assign`: ロック中のスロットへの割り当ては`classloadout.msg.slot_locked`で失敗。
  - `select`(プリセット適用)・`clear`: 新設の`LoadoutManager#applyPresetSelfService`/`#clearPersonalLoadoutSelfService`を使用。ロック中のスロットだけ現在の値を維持し、他のスロットは通常通り上書き/クリアされる(`keepLockedSlots`ヘルパーで実装)。
  - **注意**: 素の`applyPreset`/`clearPersonalLoadout`(ロックを無視する版)は強制装備パス専用として残してある——OPが強制装備する際はロックの有無に関わらず上書きできる必要があるため。
- `LoadoutSyncPacket`にレシピエント自身の`lockedSlots`(`List<LoadoutSlot>`)を追加、`LoadoutClientData.isLocked(slot)`で参照可能に。`LoadoutScreen`のスロット描画で、ロック中なら赤い`renderOutline`を追加。
- **このタイミングで発見・修正した実際のバグ**: `ServerEvents#equipLoadout`の「ホワイトリスト外のアイテムは装備しない」チェックが、ロック済み(=意図的にホワイトリストをバイパスした)スロットにも無条件でかかっていたため、ホワイトリスト外のアイテムを強制装備しても**次のリスポーンで消えていた**(装備した瞬間の1回しか効かない)。`manager.isLocked(...)`のチェックを追加してロック中は素通りするよう修正。ロック機能を実装する過程で気づいた、今セッション中に自分で作り込んだバグ。
...
`gradlew build`成功を各機能追加のたびに確認済み。**GUI/実地確認は一切未実施**(このセッションではビルド確認のみ)——特に以下は複雑な相互作用があるため優先的に確認すべき:
- 弾薬付与の複数エントリ(1アイテムに2種類設定→両方付与されるか、片方だけ削除できるか)
- 強制装備の1人/全員/チーム版それぞれ(特にチーム版はオフラインメンバーがいる場合にスキップされるか)
- 防具スロット(`helmet`等)がリスポーン時・即時装備時ともに正しく`EquipmentSlot`へ装備されるか
- ロック機能: 強制装備→本人が変更しようとして拒否される→リスポーンしても強制装備した内容が保持される(特にホワイトリスト外アイテムの場合、今回のバグ修正が効いているか)→`forceassign ... minecraft:air`で解除できるか
- ロードアウト画面の赤枠表示が実際に見えるか
...
- v0.5.2: 即時装備でも死亡時同様のインベントリクリアを実行するように変更、を含む正式リリース(2026-08-15、上記「2026-08-15セッションのまとめ」参照)
- v0.6.0: 弾薬付与の複数エントリ対応・強制装備コマンド一式(1人/全員/チーム)+専用GUI・ロードアウトスロット6→10拡張(第二ガジェット+防具4部位)・強制装備の自動ロック機能、を含む最新の正式リリース(2026-08-21、上記「2026-08-21セッションのまとめ」参照)
