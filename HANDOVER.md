...
**2026-08-10、v0.4.1としてコミット・push・リリース予定**。v0.4.0の続きのセッションで、「手持ちを追加」を除外アイテム/スポーンキット/ハンマーブロック編集画面にも拡張(画面ごとに意味が異なる、下記参照)・GUI内ホットバー表示・TACZ弾薬の名前が識別不能になるバグの補完・**プリセット保存時のクラッシュ修正(`/class save`の256文字パケット制限超過、重要)**を含む。詳細は下記「2026-08-10セッションのまとめ」節を参照。以後の変更は再びこのセクションで追跡すること。
...
- v0.4.1: 「手持ちを追加」の3画面拡張・GUI内ホットバー・TACZ弾薬名フォールバックの補完・**プリセット保存クラッシュ修正(最重要)**、を含む最新の正式リリース(2026-08-10、上記「2026-08-10セッションのまとめ」参照)
